...
private:
  ...
  std::string applied_command_topic_;  // topic from where applied (according to kinematics and stops) velocity commands are published
  ...
  ros::Publisher applied_cmd_vel_pub_; // which command is applied (depends from input, kinematic limits and status of safe and e stop)
  ...
  ros::ServiceServer use_imu_srv_;
  ...
  bool useimuSrvCallback(robotnik_msgs::enable_disable::Request& req, robotnik_msgs::enable_disable::Response& res);
};
