                              ...
                              ros::NodeHandle& controller_nh)
{
  ...
  odom_topic_ = "odom";
  applied_command_topic_ = "applied_cmd_vel";
  ...
  controller_nh.param("applied_cmd_vel", applied_command_topic_, applied_command_topic_);
  ...
  controller_nh.param<std::string>("joint/front_left_wheel_joint/name", front_left_traction_joint_,
                                   "summit_xl_front_"
                                   "left_wheel_joint");
  controller_nh.param<std::string>("joint/back_left_wheel_joint/name", back_left_traction_joint_,
                                   "summit_xl_back_left_"
                                   "wheel_joint");
  ...
  controller_nh.param("joint_speed_noise_threshold", joint_speed_noise_threshold_, 0.0);

  odom_linear_velocity_covariance_ = 0.05;
  odom_angular_velocity_covariance_ = 0.05;
  odom_position_covariance_ = 0.05;
  odom_orientation_covariance_ = 0.05;
  ...
  applied_cmd_vel_pub_ = root_nh.advertise<geometry_msgs::Twist>(applied_command_topic_, 1);
  ...
  use_imu_srv_ = controller_nh.advertiseService("use_imu", &SummitXLController::useimuSrvCallback, this);
...
void SummitXLController::update(const ros::Time& time, const ros::Duration& period)
{
  ...
  applied_cmd_vel_pub_.publish(current_cmd_);
...
void SummitXLController::limitCommand(double period)
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        ROS_WARN_STREAM_THROTTLE_NAMED(2, controller_name_,
                                       controller_name_ << ":: limiting speed ref after a E-STOP event!");
        received_cmd_.linear.x = 0.0;
        received_cmd_.linear.y = 0.0;
        received_cmd_.angular.z = 0.0;
      }
...
void SummitXLController::updateJointReferences()
{
  ...
  else
  {
    ...
    q[2] = 0.0;
    q[3] = 0.0;
    ROS_WARN_STREAM_THROTTLE_NAMED(10, controller_name_,
                                   controller_name_ << "::updateJointReferences: controller is "
                                                       "disabled");
  }
...
void SummitXLController::updateOdometry()
{
  ...
  if ((imu_watchdog_timedout_) || (!use_imu_))
  {
...
bool SummitXLController::InMotion()
{
  ...
  bool ref = (!e_stop_ && !zero_speed_ref);
  if (had_ref && !ref)
...
bool SummitXLController::useimuSrvCallback(robotnik_msgs::enable_disable::Request& req,
                                           robotnik_msgs::enable_disable::Response& res)
{
  use_imu_ = req.value;
