...
bool IcpPoseCorrector::init()
{
  ...
  pnh_.param("max_scan_delay", max_scan_delay_, 0.2);
                                                    ...
                                                    std_srvs::Trigger::Response& response)
{
  ...
  double scan_delay = (ros::Time::now() - scan_timestamp_).toSec();
  if (scan_delay > max_scan_delay_)
  {
    response.success = false;
    response.message = "Registration cannot be computed due to a high scan delay of " + std::to_string(scan_delay) + " seconds";
    ROS_WARN_STREAM(response.message);
    return true;
  }
...
void IcpPoseCorrector::initialPoseReceived(const geometry_msgs::PoseWithCovarianceStamped& unregistered_pose)
{
  ...
  if (scan_delay > max_scan_delay_)
  {
    ROS_WARN_STREAM("Registration cannot be computed due to a high scan delay of " + std::to_string(scan_delay) + " seconds");
    return;
  }
...
void IcpPoseCorrector::incomingScan(const sensor_msgs::LaserScan& scan)
{
  current_scan_ = scan;
  scan_received_ = true;
  scan_timestamp_ = scan.header.stamp;
}
...
bool IcpPoseCorrector::getCurrentPoseInMapFrame(geometry_msgs::PoseWithCovarianceStamped& pose)
{
  ...
  double scan_delay = (ros::Time::now() - current_scan_.header.stamp).toSec();

  if (scan_delay > max_scan_delay_)
  {
    ROS_WARN_STREAM("getCurrentPoseInMapFrame: cannot be computed due to a high scan delay of " + std::to_string(scan_delay) + " seconds");
    return false;
  }
