...
class SimpleCommandManager(RComponent):
    ...
    def ros_read_params(self):
        ...
        deprecated = not rospy.has_param('~base_command_list') or \
                     not rospy.has_param('~command_list')
        if deprecated == True:
            msg = "From now you may use two parameter to define the available commands " +\
                  "instead of 'command_handlers' parameter. You may use 'base_command_list' " +\
                  "to include standard commands (not specific to the application or the robot) " +\
                  "and 'command_list' to include custom commands (specific to the application or the robot)."
            rospy.logwarn("%s::ros_read_params: %s" % (self._node_name, msg))
        
        self.desired_handlers = rospy.get_param('~base_command_list', self.desired_handlers)
        
        deprecated = rospy.has_param('~command_handlers')
        if deprecated == True:
            msg = "'command_handlers' parameter is deprecated and may not work in future versions," +\
                  " please use 'command_list' instead."
            rospy.logerr("%s::ros_read_params: %s" % (self._node_name, msg))
            self.desired_handlers.extend(rospy.get_param('~command_handlers'))
        
        empty_command_list = not rospy.has_param('~command_list')
        if empty_command_list == True:
            msg = "Parameter 'command_list' not defined. You may want to add custom commands " +\
                  "(specific to the application or the robot)."
            rospy.logwarn("%s::ros_read_params: %s" % (self._node_name, msg))
            
        self.desired_handlers.extend(rospy.get_param('~command_list', []))
        
        self.pending_handlers = deepcopy(self.desired_handlers)
        self.check_command_list()

    def check_command_list(self):
        dup = {command for command in self.desired_handlers if self.desired_handlers.count(command) > 1}
        
        for command in dup:
            msg = command + " is repeated and this should not happen. Repeated command "+\
                  "can lead to unexpected behavior. Please check the parameters."
            rospy.logerr("%s::check_command_list: %s" % (self._node_name, msg))
    ...
    def ros_setup(self):
        ...
        self.get_handler_info_list_service = rospy.Service('~get_handler_info_list', GetHandlerInfoList, self.get_handler_info_list_cb)
    ...
    def ready_state(self):
        ...
        if self.command_action_server.is_active() == True:
            ...
            is_valid, self.action_result_msg = self.validate_command(goal.command.command)
            if is_valid == True:
                self.goal = goal
                ret, self.action_result_msg = self.send_cmd(self.goal.command)
                if ret != True:
    ...
    def load_handler_interface(self, handler_name):
        ...
        if len(type_list) == 2:
            if sys.version_info.major == 3:
                handler_module_prefix = "robot_simple_command_manager."
            else:
        ...
        elif 'publisher_interface' in handler_module:
            handler_module = handler_module_prefix + 'handlers.publishers.' + handler_module
        else:
    ...
    def get_handler_info_cb(self, msg):
        ...
        res.output_types = [msg.name + " is not loaded"]
        res.output_description = [msg.name + " is not loaded"]
        if msg.name in self.loaded_handlers:
            ...
            res.output_types = self.handlers_interfaces[msg.name].get_info()[2]
            res.output_description = self.handlers_interfaces[msg.name].get_info()[3]

        return res

    def get_handler_info_list_cb(self, msg):
        res = GetHandlerInfoListResponse()
        res.ret.success = False

        if self._state != State.READY_STATE:
            res.ret.message = "The component is not READY"
            res.ret.code = -1
            return res
        
        if msg.name == ['']:
            msg.name = self.loaded_handlers
        
        for handler in msg.name:
            if handler in self.loaded_handlers:
                res.ret.success = True
                handler_info = HandlerInfo()
                handler_info.name = handler
                handler_info.types = self.handlers_interfaces[handler].get_info()[0]
                handler_info.description = self.handlers_interfaces[handler].get_info()[1]
                handler_info.output_types = self.handlers_interfaces[handler].get_info()[2]
                handler_info.output_description = self.handlers_interfaces[handler].get_info()[3]
                res.handlers.append(handler_info)
            else:
                ret_msg = '%s is incorrect'%(handler)
                rospy.logwarn(('%s::get_handler_info_list_cb: %s' % (self._node_name, ret_msg)))
                res.ret.message+=ret_msg + " "
                res.ret.code = -1
    ...
    def send_cmd(self, msg):
        ...
        try:
            command_sent,handler_msg = self.handlers_interfaces[self.current_handler].send_command(
                parsed_command[1:])

            full_command = msg.command
            if command_sent:
                full_command = self.handlers_interfaces[self.current_handler].get_command()
                ret_msg = "Command '%s' sent!" % (full_command)
                rospy.loginfo('%s::send_cmd: %s' % (self._node_name, ret_msg))
            else:
                ret_msg = "Error sending the command '%s': %s" % (full_command, handler_msg)
                rospy.logerr('%s::send_cmd: %s' % (self._node_name, ret_msg))

        except Exception as e:
            ret_msg = "Error parsing command '%s': %s" % (msg.command, e)
            rospy.logerr('%s::send_cmd: %s' % (self._node_name, ret_msg))
    ...
    def build_result(self, code):
        ...
        output = self.handlers_interfaces[self.current_handler].get_output()
    ...
    def validate_command(self, command):
        ...
        if parsed_handler not in self.loaded_handlers:
            return False, "Command tag " + parsed_handler + " does not exist"
