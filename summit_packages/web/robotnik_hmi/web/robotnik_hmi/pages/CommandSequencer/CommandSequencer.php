                            ...
                            <div class="row">
                                ...
                                <div class="col-xs-6 col-ms-6 col-md-9 col-xl-9">
                                    <div class="row">
                                        ...
                                        <div id="networkContainer" style="border: 1px solid #ccc;height: 500px">
...
<script>
    ...
    var KS_START_LIST = "["
    var KS_END_LIST = "]"
    function jointList(args)
    {
        var merged_args = [];
        var merged_arg = "";
        var in_lists_counter = 0;
        for (let i = 0; i < args.length; i++) {
            argument = args[i];
            if (argument.includes(KS_START_LIST))
            {
                in_lists_counter += 1;
            }
            if (argument.includes(KS_END_LIST))
            {
                in_lists_counter -= 1;
            }
            merged_arg += " " + argument;

            if (in_lists_counter == 0)
            {
                merged_args.push(merged_arg.trim());
                merged_arg = "";
            }
        }
        return merged_args;
    ...
    function addElementInVisualization(command, element_id=-1){
        ...
        var command_split = jointList(command.split(" "));
    ...
    function updateVisualElements (handlers_information){
        ...
        for(it_select_handler_info; it_select_handler_info<handlers_information.length; it_select_handler_info++)
        {
            ...
            if(types == -1){
                ...
                setData(command_sequences_information_visual_);
                return false;
    ...
    function setData(commands){
        const data = {
            nodes: new vis.DataSet([]),
            edges: new vis.DataSet([]),
        };
        const container = document.getElementById("VisualizationActionDiv");

        gph_ = new vis.Network(container, data, options);
        gph_.on("click", function (params){onclickElementNetwork(params)});
        gph_.canvas.frame.addEventListener("wheel", function (event) {
            event.preventDefault();
            var view_position = gph_.getViewPosition();
            var sign = 1;
            if (event.deltaY < 0){
                sign = -1;
            }
            gph_.moveTo({position: {x: view_position.x, y: view_position.y + (10 * sign)}});
            gph_.redraw();
        ...
        gph_.setSize(container.offsetWidth, container.offsetHeight);
        // Get the current view of the network
        const current_view = gph_.getViewPosition();
        const current_scale = gph_.getScale();


        for (var i=0; i < commands["nodes"].length; i++){
            data.nodes.add(commands["nodes"][i]);
        }


        for (var i=0; i < commands["edges"].length; i++){
            data.edges.add(commands["edges"][i]);
        }

        var last_node = commands["nodes"].length - 1
        gph_.redraw();
        //gph_.moveTo({scale: current_scale });
        //gph_.focus("Init_01_____0", {scale: 1, offset: {x: 0, y: 0}, locked: false, animation: false});        
        gph_.focus(commands["nodes"][last_node].id, {scale: 1, offset: {x: 0, y: 0}, locked: false, animation: false});        
    }
    ...
    async function getHandlers() {
        try {
            ...
            sorted_handlers = res["handlers"].sort();
            handlers_type_ = [];
            for (handler_name_foreach of sorted_handlers) {
                srv = { name: handler_name_foreach };
                get_handler_info_res = await new Promise((resolve, reject) => {
                ...
                for (let i = 0; i < get_handler_info_res["description"].length; i++) {
                    values = {
                        description: get_handler_info_res["description"][i],
                        type: get_handler_info_res["types"][i],
                    };
    ...
    function chargeParamPanel(type_of_command, previous_action="Init", edit=false, values=[]){
        ...
        if(values.length != handler.length){
            ...
            jQuery.each(handler, function( handler_type_index, handler_type_foreach ){
                ...
                else if(type_of_command.includes("PTZ_TAG") && handler_type_foreach["description"].includes("environment")){
                    values.push(current_environment);
                }
        ...
        for(i=0; i < handler.length; i++){
            if(type_of_command.includes("GOTO_TAG") && handler[i]["description"].includes("tag")){
            ...
            }else if(type_of_command.includes("PTZ_TAG") && handler[i]["description"].includes("tag")){
                element = document.getElementById("ListDiv").cloneNode(true);
                element.id = "elem_01_____" + i;
                element.children[0].children[0].setAttribute("for", "value_command_01_____"+i);
                element.children[0].children[0].textContent=handler[i]["description"]+":";
                ...
                //element.children[0].children[1].value=values[i];
                element.children[0].children[1].disabled=!edit;
            }else if(type_of_command.includes("PTZ_TAG") && handler[i]["description"].includes("environment")){
                element = document.getElementById("StringDiv").cloneNode(true);
                element.id = "elem_01_____" + i;
                element.children[0].children[0].setAttribute("for", "value_command_01_____"+i);
                element.children[0].children[0].textContent=handler[i]["description"]+":";
    ...
    function saveSequence(){
        ...
        if(command_sequences_information_visual_["nodes"].length == 1 || command == ""){
