                                                ...
                                                <div class="col-xs-8 col-ms-8 col-md-8 col-xl-8">
                                                  <div class="input-group ">
                                                    ...
                                                    <div class="col-xs-12 col-ms-12 col-md-12 col-xl-12 text-right">
                                                        <button type="button" class="btn btn-default" onclick="setMapAsDefault()"><span class="glyphicon glyphicon-ok-sign"></span> Set map as default</button>
                                                    </div>
                                                </div>

                                            </div>
                                        </div>
                                        <div class="row" id="mapControlProccesingNewMapDiv" hidden="true">
                                            <div class="col-xs-12 col-ms-12 col-md-12 col-xl-12">
                                                <label>Proccessing map, this may take a few minutes.</label><label id="labelEstimateTime"></label>
                                            </div>
                                            <div class="col-xs-12 col-ms-12 col-md-12 col-xl-12">
                                                <div class="progress">
                                                    <div class="progress-bar progress-bar-info progress-bar-striped active" role="progressbar" id="progressProccesingNewMapDiv" aria-valuenow="70" aria-valuemin="0" aria-valuemax="100">
                                                            <label id="progressProccesingNewMapLabel">0</label></p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div id="divSafetytatus" class="tab-pane fade">
                                            ...
                                            <div class="row">
												<div class="col-xs-6 col-ms-6 col-md-6 col-xl-6">
                                                        <div class="row">
                                                            <div class="col-xs-6 col-ms-6 col-md-6 col-xl-6">
                                                                <label><?php echo $lang['MONITOR_SAFETY_OPERATION_MODE']; ?>:</label>
                                                            </div>
                                                            <div class="col-xs-6 col-ms-6 col-md-6 col-xl-6">
                                                                <label id="labelSafetyOperationMode"><?php echo $lang['TXT_UNKNOWN'];?></label>
                                                            </div>
                                                        </div>
                                                </div>
                                                <div class="col-xs-6 col-ms-6 col-md-6 col-xl-6">
...
function setMapAsDefault()
{
    service_ns = '<?php echo $robot_dictionary["rlc_localization_ns"]?>set_default_module';
    srv = {
        module_name: "",
        environment: ""
    }
    callService(service_ns, 'robot_local_control_msgs/SetEnvironment', srv, function (res) {
        ...
        {
            alertify.error("ERROR to set as default: " + res.message);
        }
    });
}
...
var assets_writer_progress = null;
var previus_assets_writer_progress = null;
var assets_writer_progress_last_time = null;
var estimate_time_assets_write_progress = null;
var last_value_to_estimate_time = null;
subscribe('/robot/assets_writer_progress', 'cartographer_ros_msgs/AssetsWriterProgress', function (message) {
    if(assets_writer_progress_last_time!=null)
    {
        current_time = new Date();
        diff_millis = current_time.getTime() - assets_writer_progress_last_time.getTime();
        diff_seg = diff_millis / 1000;
        current_progress = message.progress;
        last_progress = assets_writer_progress.progress;
        diff_progress = (current_progress-last_progress);
        estimate_time_assets_write_progress = ((diff_seg/diff_progress)*(1-current_progress)).toFixed(0)
    }
    else
    {
        estimate_time_assets_write_progress = null;
    }
    assets_writer_progress = message;
    assets_writer_progress_last_time = new Date();
    last_value_to_estimate_time = message;
});
...
subscribe('<?php echo $robot_dictionary["topic_robot_local_state"];?>', 'robot_local_control_msgs/Status', function (message) {
    ...
    if(has_safety){
        ...
        $('#labelSafetyOperationMode').text(safety.operation_mode);
...
subscribe('<?php echo $robot_dictionary["topic_hw_status"]?>', 'robotnik_msgs/RobotnikMotorsStatus', function (message) {
    if(readyOk)
    {
        ...
        table_motors.clear();
        message.motor_status.forEach(function (item, index){
            var data_motor = {};
            data_motor["name"] = item.joint;
            data_motor["can_id"] = item.can_id;
            data_motor["state"] = item.state;
            data_motor["status"] = item.status;
...
// update odometry
  ...
  if (robot_local_control_status != null){
    ...
    if(robot_local_control_status.localization_status.type != ''){
      ...
      if(robot_local_control_status.localization_status.type == 'localization_2d')
      {
        $("#saveMapRow").hide();
      ...
      else if(robot_local_control_status.localization_status.type == 'localization_3d')
      {
        $("#saveMapRow").hide();
        $("#environmentsRow").show();
        
      }
      else if(robot_local_control_status.localization_status.type == 'mapping_2d')
      {
        $("#saveMapRow").show();
        $("#environmentsRow").hide();
      }
      else if(robot_local_control_status.localization_status.type == 'mapping_3d'  || assets_writer_progress!=null)
      {
        $("#environmentsRow").hide();
        if(assets_writer_progress!=null && assets_writer_progress_last_time!=null)
        {
            current_time = new Date(); 
            diff_millis = current_time.getTime() - assets_writer_progress_last_time.getTime();
            if(diff_millis <= 6000)
            {
                $("#saveMapRow").hide();
                $("#progressProccesingNewMapLabel").text((assets_writer_progress.progress*100).toFixed(2)+"%");
                $("#progressProccesingNewMapDiv").css("width", (assets_writer_progress.progress*100)+"%");
                $("#button_save_map").prop('disabled', true);
                $("#mapControlProccesingNewMapDiv").show();
                if(estimate_time_assets_write_progress!=null)
                {
                    if(estimate_time_assets_write_progress < 60)
                    {
                        $("#labelEstimateTime").text("Estimated:" + estimate_time_assets_write_progress + " seg");
                    }
                    else
                    {
                        $("#labelEstimateTime").text("Estimated:" + (estimate_time_assets_write_progress/60).toFixed(0) + " min");
                    }
                }
                else
                {
            ...
            {
                $("#saveMapRow").show();
                $("#progressProccesingNewMapDiv").css("width", "0%");
                $("#progressProccesingNewMapLabel").text("0"+"%");
                $("#progressProccesingNewMapDiv").val("0%");
                $("#labelEstimateTime").text("");
                $("#button_save_map").prop('disabled', false);
                $("#mapControlProccesingNewMapDiv").hide();
                assets_writer_progress=null;
                assets_writer_progress_last_time=null;
            }
        }
        else
        {
