...
<html>
    <head>
        ...
        <script>
            ...
            // Get subpath and add it to the address. This is to support the case when the app is not in the root of the server
            var currentPath = window.location.pathname;
            // Remove index.php of the path
            currentPath = currentPath.replace('index.php', '');
            var pathArray = currentPath.split('/');
            for (i = 0; i < pathArray.length; i++) {
                if (pathArray[i] != "") {
                    address = address + '/' + pathArray[i];
                }
            }
